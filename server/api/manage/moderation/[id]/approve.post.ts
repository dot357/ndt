...
export default defineEventHandler(async (event) => {
  ...
  await enforceRateLimit(event, { name: 'manage-approve', max: 40, windowMs: 60_000, key: user.id })

  const body = await readBody<Body>(event)

  const { error } = await client.from('proverbs').update({ status: 'published' }).eq('id', proverbId)
  if (error) throw createError({ statusCode: 500, statusMessage: 'Internal Server Error', message: error.message })

  await logModAction(client, user.id, 'approve', 'proverb', proverbId, body?.note || null)
  return { ok: true }
})
