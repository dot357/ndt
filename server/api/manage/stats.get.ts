...
export default defineEventHandler(async (event) => {
  ...
  if (users.error || proverbs.error || reactions.error || reports.error || actions.error) {
    const message = users.error?.message || proverbs.error?.message || reactions.error?.message || reports.error?.message || actions.error?.message || 'Failed to fetch stats.'
